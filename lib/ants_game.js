...
class AntColony {
  ...
  static createDefaultColony() {
    return new AntColony(99, 1,8,0); //change this back to 2 food later
  }
...
var Ants = {
  ...
  Hungry : class extends Ant {
    ...
    act(AntGame) {
        var target = this.place.getClosestBee(0,10); // if current turn is > eaten + 3
        if (target) {
            target.reduceArmor(target.armor);
            console.log(this + ' ate '+target + ' and must take three turns to digest');
            var eaten = AntGame.turn; // this is undefined for some reason, fix later
            console.log(eaten);
        }
      }     
  },
  ...
  Fire : class extends Ant {
    ...
    act() {
        // does nothing, but when armor = 0, deals 3 dmg of all bees in the same place w/ reduceArmor()
      } 
      /*
      reduceArmor(amount) {
        this._armor -= amount;
  ...
  Scuba : class extends Ant {
    ...
    act() {
        // throw leaves but is water safe
      }        
  },
  ...
  Ninja : class extends Ant {
    ...
    act() {
        // hurts all bees that run pasts it by 1
      }        
  },
