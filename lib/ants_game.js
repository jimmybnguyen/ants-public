...
class AntGame {
  ...
  takeTurn() {
    ...
    this._colony.allAnts.forEach(function(ant){
      ant.act(this._colony,this._turn); //pass in colony & turn reference if needed
    }, this);
...
var Ants = {
  ...
  Hungry : class extends Ant {
    ...
    act(game, turn) {
        if (eaten == null || turn > eaten + 3) { // does not work properly yet 
  ...
  Fire : class extends Ant {
    ...
    act() {
        // grabs all of the bees in the same place and deals damage to them
        var bees = this.place.bees;
        for (var i = bees.length - 1; i >= 0; i--) { //var i in bees
            var target = bees[i];
            target.reduceArmor(this._damage);
