...
var Ants = {
  ...
  Queen : class extends Ant {
      ...
      constructor()  {
          ...
          this._watersafe = true;
          this._antsDoubled = [];
          //this._numberofQueens++;
          //var original = true;
          //if (this._numberofQueens > 1) {
            //original = false;  
          //}
    }
    ...
    act(colony) {
        ...
        var curr = this.place.entrance.ant;
        var isDoubled = false;
        for (var i = 0; i < this._antsDoubled.length; i++) {
            if (this._antsDoubled[i] == curr) {
                isDoubled = true;   
            }
        }
        if (!isDoubled && curr != null) {
            curr.damage *= 2;
            this._antsDoubled.push(curr); 
        }
        // change back to false to take care of back ant
        if (isDoubled) {
            !isDoubled;    
        }
        curr = this.place.exit.ant;
         for (var i = 0; i < this._antsDoubled.length; i++) {
            if (this._antsDoubled[i] == curr) {
                isDoubled = true;   
            }
